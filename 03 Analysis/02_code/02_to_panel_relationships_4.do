...
/* 
FROM CODEBOOK:
This data set is only available in the long version of the questionnaire 
and only for respondents who have had since January 2011 or 
currently have a romantic relationship (y6_date2=1).
...
* Generating variable of individuals who ever had a relationship after 2011
recode _merge (3= 1 "Had relationship") (2 =0 "Never relationship"), gen(ever_relationship)
drop _merge 

* Some individuals have missing response - exclude
replace ever_relationship = .  if y6_date2 == -66
drop if ever_relationship == .

ta ever_relationship // 1,524 cases never had a relationship
...
* Note: we assume that few missing cases are natives
recode y6_rpback2RV ///
(-99 -88 -55 = 1 "Native") (2 =2 "Foreign") , gen(spelltype)
...
* Checking cases with missing dates
mdesc begincm endcm
mdesc begincm endcm if ever_relationship==1

* Drop those that are not cases who never had relationships
cap drop flag*
gen flag = ever_relationship==1 & (missing(begincm) | missing(endcm))
bysort youthid: egen flag_max = max(flag)
drop if flag_max==1
...
* Keeping some varaibles 
keep youthid ever_relationship index spelltype ///
birth_year begincm endcm birthcm intcm ///
minbegincm maxendcm ongoing birthdy birthdm
...
* Checking within individual
bysort youthid: egen id_minbegincm = min(begincm) 
...
* Create a "blanco" dataset with youthid and N total observations 
*(see dis  maxendcm -  minbegincm) to merge the spelldata with 
...
* MERGING PANEL WITH SPELLS AND FILLING EMPTY PERIODS
********************************************************************************
...
* Dropping the unmerged with never relationship (bc don't have dates)
drop if _merge==2
...
* Dealing with observations after interview
replace spelltype = -2 if begincm>intcm


* Dealing with those with never relationship
	* Single/missing before 2011
	* Single from 2011
replace spelltype=-1 if ever_relationship==0 & begincm<ym(2011,1)
replace spelltype = 0 if ever_relationship==0 & begincm>=ym(2011,1) & begincm<=intcm
ta spelltype if ever_relationship==0, miss


* Dealing with those who have had a relationship

bysort youthid: egen youth_minbegin = max(id_minbegincm)

* First relationship before 2011: Missing-single before
replace spelltype = -1 if ///
	ever_relationship==1 & ///
	youth_minbegin <= ym(2011,1) & ///
	begincm < youth_minbegin 

* First relationship after 2011: single from 2011 to beginning
replace spelltype = 0 if ///
	ever_relationship==1 & ///
	youth_minbegin > ym(2011,1) & ///
	begincm < youth_minbegin & ///
	begincm >= ym(2011,1)	

* First relationship after 2011: missing-single before 2011
replace spelltype = -1 if ///
	ever_relationship==1 & ///
	youth_minbegin > ym(2011,1) & ///
	begincm < youth_minbegin & ///
...
* Generating the other singlehood spells (in between relationships)
replace spelltype = 0 if ///
	ever_relationship==1 & ///
	begincm > youth_minbegin & ///
	spelltype == .
...
duplicates drop youthid begincm endcm index, force

* Relabelling the spells
label define spelltype -1 "Missing/single" -2 "Post-interview" ///
	0 "Single" 1 "Native-origin partner" 2 "Migrant-origin partner" ///
	3 "Cohabiting", replace
...
* Version 2: missing/single coded as single
destring spell_rel, ignore(".") replace
recode spell_rel (0 -1 = 0 "Single") ///
(1 = 1 "Rel: nat-origin") (2 = 2 "Rel: mig-origin") ///
(13 31 3 = 3 "Cohab: nat-origin") (23 32 = 4 "Cohab: mig-origin") ///
(-2 = -2 "Post-interview") , ///
gen(spell_relationship_2) label(spell_rel_2)

ta spell_relationship_2
